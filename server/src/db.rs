...
impl UserExt for DBClient {
    ...
    async fn search_by_email(
        ...
        let users = sqlx::query_as!(
            User,
            r#"SELECT id, name, email, password, public_key, created_at, updated_at
    ...
    async fn get_sent_files(
        ...
        let count_row = sqlx::query_scalar!(
            r#"SELECT COUNT(*) FROM shared_links sl
            JOIN files f ON sl.file_id = f.id WHERE f.user_id = $1"#,
            user_id
        )
        .fetch_one(&self.pool)
        .await?;
